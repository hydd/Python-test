...
def genData():
    data=pd.read_csv('tags.csv',header=None,sep=',')
    data.columns=['user','item','label','date']
...
if __name__=='__main__':
    ...
    M=buildMatrix_M(G)
    AA=before_GetRec(M)
    recall, precision, coverage, popularity, diversity=evaluate(train, test, AA, M, G, alpha, N, item_tags, user_items)